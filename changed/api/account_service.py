...
class AccountService:
    ...
    def create_account_and_tenant(
        ...
        TenantService.create_owner_tenant_if_not_exist(account=account, inviter_id=None)
...
class TenantService:
    ...
    def create_owner_tenant_if_not_exist(
            account: Account, name: Optional[str] = None, is_setup: Optional[bool] = False, inviter_id: Optional[str] = None
    ) -> object:
        ...
        if inviter_id:
            first_account = Account.query.order_by(Account.created_at.asc()).first()
            print(inviter_id)
            print(first_account)
            if not first_account or inviter_id != first_account.id:
                return
        """
        first_account = Account.query.order_by(Account.created_at.asc()).first()
        if first_account and account.id != first_account.id:
            tenant_to_delete = db.session.query(Tenant).join(TenantAccountJoin, Tenant.id == TenantAccountJoin.tenant_id)
            tenant_to_delete = tenant_to_delete.filter(
                TenantAccountJoin.account_id == account.id,
                TenantAccountJoin.role == TenantAccountJoinRole.OWNER.value,
                Tenant.name == f"{account.name}'s Workspace"
            ).first()
            if tenant_to_delete:
                db.session.query(TenantAccountJoin).filter_by(tenant_id=tenant_to_delete.id).delete()
                db.session.delete(tenant_to_delete)
                db.session.commit()

        if not first_account or account.id == first_account.id:
            pass
        elif inviter_id and inviter_id == str(first_account.id):
            pass
        else:
            return
...
class RegisterService:
    ...
    def setup(cls, email: str, name: str, password: str, ip_address: str) -> None:
        ...
        try:
            ...
            TenantService.create_owner_tenant_if_not_exist(account=account, is_setup=True, inviter_id=None)
    ...
    def invite_new_member(
        ...
        token = cls.generate_invite_token(tenant, account, inviter=inviter)
    ...
    @classmethod
    def generate_invite_token(cls, tenant: Tenant, account: Account, inviter: Optional[Account] = None) -> str:
        ...
        print("invitation_data:", invitation_data)
        return token
...
def should_create_personal_workspace(account, invite_token=None):
    first_account = Account.query.order_by(Account.created_at.asc()).first()
    ...
    if invite_token:
        ...
        if invitation:
            invited_workspace_id = invitation.get("workspace_id")
            if invited_workspace_id:
                from models.account import TenantAccountJoin
                inviter_join = TenantAccountJoin.query.filter_by(tenant_id=invited_workspace_id, role="owner").first()
                if inviter_join and inviter_join.account_id == first_account.id:
                    return True
    return False
